...
function mergeInfoObjects(results) {
  const merged = {};
...
function aggregateResults(results, fallbackError, type) {
  const successfulResults = results.filter(result => result.success);
  const preferredResult = pickPreferredResult(successfulResults);

  if (!preferredResult) {
    return { success: false, error: fallbackError, type };
  }

  const mergedInfo = mergeInfoObjects(successfulResults);
  const mergedSources = successfulResults.map(result => result.source);
...
async function queryIP(ip) {
  ...
  const domesticAggregated = aggregateResults(domesticResults, '国内查询失败', 'domestic');
  const foreignAggregated = aggregateResults(foreignResults, '国外查询失败', 'foreign');
  
  return {
    success: true,
    domestic: domesticAggregated,
    foreign: foreignAggregated,
